...
/*
	...
	속성
		...
		contentType: 전송시 사용 할 컨텐츠 타입(get과 post가 다름)
	...
	*/
	...
	$('#ajaxBtn').click(function() {
		$.ajax({
			...
			data : {
				msg : $(this).text(),
				varStr : "$.ajax()메소드 매우 좋다요"
			},
			success : sucFunc, /* sucFunc(data) 형태로 인자를 사용하지 않는다. */
			error : errFunc
		});
	});
});
...
/*
	콜백메소드를 외부함수 형태로 정의함. 해당 함수를 호출시에는 함수명 만으로 호출한다.
	매개변수는 사용하지 않는다.
		error: errFunc => O
...
/*
	요청 성공시 호출할 콜백메소드. 
	외부함수로 정의되어 있고 함수명만으로 호출하더라도 콜백데이터는 파라미터로 받을 수 있다.
 */
