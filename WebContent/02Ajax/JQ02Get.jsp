...
$.get()
	...
	사용법
		...
		- success(data): 요청이 성공했을 때 실행된 콜백(callback)메소드.
			콜백메소드가 호출될 때 전달되는 파라미터를 통해 성공 혹은 실패여부를 판단할 수 있다.
...
$(function(){
	...
	/*
	파라미터 조립하기
	: $.get() 혹은 $.post()를 통해 서버로 요청을 하는 경우
	파라미터를 전송할 때는 항상 JSON으로 조립해야 한다.
	하지만 폼값이 많아서 조립이 힘든경우에는 serialize()를 사용한다.
	
	$('폼이름').serialize()
	-> 해당 메소드를 이용하면 form하위의 모든 요소에 대해 JSN으로 조립해준다.
	단, input요소의 name 속성값이 key로 사용된다는 것에 주의하자.
	*/
});
